...
xfit = np.arange(0, np.max(y_pred [:,-1]))
def fit(x):
    return x
plt.plot(xfit, fit(xfit), color='black')
plt.scatter(np.array(features_y)[:,-1],y_pred[:,-1],color='blue',s=8,label='Train')
...
#
# P_pd = pd.DataFrame(features_y)
# y_pd = pd.DataFrame(y_pred)
# writer = pd.ExcelWriter('IIa.xls')
# y_pd.to_excel(writer, 'ture')
# P_pd.to_excel(writer, 'pre')
# writer.save()
# writer.close()
