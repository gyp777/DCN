...
# ('----------------------data----------------------')
data = pd.read_csv('data_DCNII.csv')
features_name = list(data)
features_name_x = features_name[:15]
features_name_y = features_name[15:]
...
class CrossLayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for i in range(self.num_layer):
            self.W.append(
                self.add_weight(shape=[1, features_x.shape[1] + n_s + n_t + n_b - 3], initializer='glorot_uniform',
                                name='w_{}'.format(i),
                                trainable=True))
            self.bias.append(
                self.add_weight(shape=[1, features_x.shape[1] + n_s + n_t + n_b - 3], initializer='zeros',
                                name='b_{}'.format(i), trainable=True))
        self.built = True
        ...
        for i in range(self.num_layer):
            if i == 0:
                cross = tf.keras.layers.Lambda(
                    lambda x: K.batch_dot(K.dot(x, K.transpose(self.W[i])), x) + self.bias[i] + x)(input)
            else:
                cross = tf.keras.layers.Lambda(
                    lambda x: K.batch_dot(K.dot(x, K.transpose(self.W[i])), input) + self.bias[i] + x)(cross)
                # print(cross)
...
model = load_model('trained_DCNII.h5', custom_objects={'my_loss': my_loss, "CrossLayer": CrossLayer})
...
plt.loglog(xfit, fit(xfit), color='black')
plt.loglog(np.array(features_y)[:, -1], y_pred[:, -1], '.', color='blue')
